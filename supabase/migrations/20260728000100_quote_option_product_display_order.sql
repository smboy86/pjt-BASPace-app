...
drop index public.quote_option_products_option_sort_idx;

create index quote_option_products_option_sort_idx
  on public.quote_option_products (quote_option_id, display_order, created_at, id);
...
begin
  ...
  if exists (
    select 1
    from jsonb_to_recordset(target_products)
      as product(id uuid, name text, price bigint, image_path text, created_at timestamptz, display_order integer)
  ...
  where product.quote_option_id = target_option_id and product.image_path is not null
    and not exists (
      select 1
      from jsonb_to_recordset(target_products)
        as next_product(id uuid, name text, price bigint, image_path text, created_at timestamptz, display_order integer)
