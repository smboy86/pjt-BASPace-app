...
**검수 범위 규칙:**
- spec에서 켜진 항목만 검수 (꺼진 항목의 화면/모듈은 존재하면 오히려 FAIL로 보고)
- `*_notes`의 제약(예: "온보딩 3장 이하", "인터스티셜 금지")이 화면/동작에 그대로 반영됐는지 확인
- spec의 `auth.account_deletion`, `policy.ugc` 등 정책 의무 항목은 화면/흐름 단위로 누락 여부 확인 (Apple 5.1.1(v) / 1.2)
- 모순 발견 시 `*_notes` 우선이 원칙

## 작업 원칙
- **PRD 기반 검수** — `_workspace/plan/prd.md`의 유저 스토리를 체크리스트로 변환
- **디자인 시스템 대조** — `_workspace/design/design-system.md`의 토큰 값과 실제 코드 비교
- **코드 레벨 검증** — 실제 컴포넌트 파일을 읽어 className, props, 상태 처리 확인
- **경계면 교차 비교** — API 타입과 UI 타입이 일치하는지, 스토어 상태와 화면이 동기되는지
- **spec 일치 검수** — spec.md의 켜진/꺼진 항목과 실제 구현이 일치하는지 매핑 검수
- **런타임 검증** — 코드만 읽지 말고, 가능하면 시뮬레이터에서 앱을 실제로 실행해 화면·흐름을 조작 검증한다 (아래)

## 런타임 시뮬레이터 검증 (sim-use)

정적 코드 검수만으로 잡히지 않는 실제 동작(네비게이션 연결, 빈/에러/로딩 상태 전환, 키보드 회피, 오클릭)은 시뮬레이터에서 직접 확인한다. **CLAUDE.md의 "시뮬레이터 런타임 테스트 (sim-use)" 섹션이 정본**이다.

1. **부트스트랩** — 검증 시작 전 sim-use 설치 확인, 없으면 설치:
   ```bash
   if ! command -v sim-use >/dev/null 2>&1; then
     brew tap lycorp-jp/tap && brew install lycorp-jp/tap/sim-use
     sim-use init --client claude --dest .claude/skills
   fi
   ```
   (CLT 미갱신 등으로 설치 실패 시, 런타임 검증은 skip으로 표시하고 정적 검수만 진행 — 보고서에 명시한다.)
2. **observe → act → verify** — `sim-use ui`로 화면을 관찰(@별칭)하고, `tap @N`/`type`/`swipe`로 조작한 뒤 다시 `sim-use ui`로 결과를 확인한다. 좌표가 아닌 요소 별칭으로만 조작한다.
3. PRD 유저 스토리의 happy path를 시뮬레이터에서 직접 밟아 PASS/FAIL을 판정하고, 발견한 런타임 이슈는 스크린샷(`sim-use screenshot`)과 함께 보고서에 기록한다.
...
### 광고 배치 (monetization에 광고 포함 시)
- [ ] 배너가 버튼/탭바/FAB 등 터치 요소와 충분히 이격되어 있는가 (오클릭 방지 — 최소 16dp)
- [ ] 화면당 배너가 1개인가
- [ ] 배너 미로드 시 빈 placeholder 영역이 남지 않는가 (collapse 처리)
- [ ] 빈 상태/에러/로딩 화면에 광고가 노출되지 않는가
- [ ] 광고가 다른 UI에 가려지거나 콘텐츠를 가리지 않는가
- [ ] 전면 광고가 사용자 탭 직후가 아닌 자연스러운 break point에서 노출되는가
- [ ] 전면 광고가 연속으로(dismiss 직후) 노출되지 않는가
- [ ] 리워드 광고가 opt-in 버튼으로만 시작되고 보상 내용이 명시되어 있는가
